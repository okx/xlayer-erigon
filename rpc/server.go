...
func (s *Server) ServeCodecWithAPIKey(codec ServerCodec, options CodecOption, apikey string) {
	...
	// X Layer api key
	c := initClient(codec, s.idgen, &s.services, s.logger, apikey)
	<-codec.closed()
	c.Close()
}
...
func (s *Server) serveSingleRequest(ctx context.Context, codec ServerCodec, stream *jsoniter.Stream) {
	...
	// X Layer api key
	h := newHandler(ctx, codec, s.idgen, &s.services, s.methodAllowList, s.batchConcurrency, s.traceRequests, s.logger, s.rpcSlowLogThreshold, ctx.Value("apikey").(string))
