...
get_latest_l2_batch() {
    local latest_block
    latest_block=$(cast block latest --rpc-url "http://127.0.0.1:8123" | grep "number" | awk '{print $2}')
    local latest_batch
    latest_batch=$(cast rpc zkevm_batchNumberByBlockNumber "$latest_block" --rpc-url "http://127.0.0.1:8123" | sed 's/^"//;s/"$//')
...
wait_for_l1_batch() {
    ...
    while true; do
        ...
        if [ "$batch_type" = "virtual" ]; then
            ...
            current_batch=$(cast logs --rpc-url "http://127.0.0.1:8545" --address 0xeb173087729c88a47568AF87b17C653039377BA6 --from-block 0 --json | jq -r '.[] | select(.topics[0] == "0x3e54d0825ed78523037d00a81759237eb436ce774bd546993ee67a1b67b6e766") | .topics[1]' | tail -n 1 | sed 's/^0x//')
            current_batch=$((16#$current_batch))
        elif [ "$batch_type" = "verified" ]; then
            current_batch=$(cast rpc zkevm_verifiedBatchNumber --rpc-url "http://127.0.0.1:8124" | sed 's/^"//;s/"$//')
        else
...
sed -i \
-e 's/zkevm\.executor-strict: true/zkevm.executor-strict: false/' \
-e 's/zkevm\.executor-urls: zkevm-stateless-executor-001:50071/zkevm.executor-urls: ","/' \
-e '$a zkevm.disable-virtual-counters: true' \
./config/test.erigon.seq.config.yaml
...
polycli loadtest --rpc-url "http://127.0.0.1:8124" --private-key "0x815405dddb0e2a99b12af775fd2929e526704e1d1aea6a0b4e74dc33e2f7fcd2" --verbosity 600 --requests 2000 --rate-limit 500  --mode uniswapv3 --legacy
...
cast send "0x2d42E2899662EFf08b13eeb65b154b904C7a1c8a" "rollbackBatches(address,uint64)" "0xeb173087729c88a47568AF87b17C653039377BA6" "$latest_verified_batch" --private-key "0x815405dddb0e2a99b12af775fd2929e526704e1d1aea6a0b4e74dc33e2f7fcd2" --rpc-url "http://127.0.0.1:8545"
...
sequencer_hash=$(cast block $comparison_block --rpc-url "http://127.0.0.1:8123" | grep "hash" | awk '{print $2}')

# wait for block to be available on sync node
if ! wait_for_l2_block_number $comparison_block "http://127.0.0.1:8124"; then
    echo "Failed to wait for batch verification"
    exit 1
fi

echo "Getting block hash from node"
node_hash=$(cast block $comparison_block --rpc-url "http://127.0.0.1:8124" | grep "hash" | awk '{print $2}')
